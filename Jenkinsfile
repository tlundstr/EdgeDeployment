				...
				container(name: 'dind', shell: '/bin/sh') {
					...
					script {
						docker.withRegistry("${REGISTRY_INGRESS}") {
							...
							def customImage = docker.build("${CONTAINER}:${CONTAINER_TAG}", "${PACKAGE}/build/container --no-cache --build-arg EDGE_VERSION=${EDGE_VERSION} --build-arg WPM_CRED=${WPM_CRED} --build-arg GITHUB_CREDS_USR=${GITHUB_CREDS_USR} --build-arg WPM_CRED_PSW=${GITHUB_CREDS_PSW}")
