				...
				container(name: 'dind', shell: '/bin/sh') {
					...
					script {
						docker.withRegistry("${REGISTRY_INGRESS}") {
							...
							def customImage = docker.build("${CONTAINER}:${CONTAINER_TAG}", "${PACKAGE}/build/container --no-cache --build-arg EDGE_VERSION=${EDGE_VERSION} --build-arg WPM_CRED=${WPM_CRED} --build-arg WPM_CRED_USR=${WPM_CRED_USR} --build-arg WPM_CRED_PWS=${WPM_CRED_PWS}")
